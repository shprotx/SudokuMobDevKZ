...
internal fun DrawScope.drawTiara(center: Offset, r: Float, accent: Color) {
    val baseY = center.y + r * 0.42f
    val halfW = r * 0.92f

    val band = Path().apply {
        moveTo(center.x - halfW, baseY)
        cubicTo(
            center.x - halfW * 0.4f, baseY + r * 0.22f,
            center.x + halfW * 0.4f, baseY + r * 0.22f,
            center.x + halfW, baseY,
        )
        cubicTo(
            center.x + halfW * 0.4f, baseY + r * 0.36f,
            center.x - halfW * 0.4f, baseY + r * 0.36f,
            center.x - halfW, baseY,
        )
        close()
    ...
    drawPath(
        path = band,
        brush = Brush.linearGradient(
            colors = listOf(silverMetal.light, silverMetal.mid),
            start = Offset(center.x, baseY),
            end = Offset(center.x, baseY + r * 0.36f),
        ),
    )
    drawPath(path = band, color = silverMetal.outline, style = Stroke(width = r * 0.04f))

    val arcs = listOf(
        Triple(-0.62f, -0.18f, 0.34f),
        Triple(0f, -0.78f, 0.5f),
        Triple(0.62f, -0.18f, 0.34f),
    )
    arcs.forEach { (dx, dyTop, halfSpread) ->
        val peak = Offset(center.x + dx * halfW, center.y + dyTop * r)
        val leftBase = Offset(center.x + (dx - halfSpread) * halfW, baseY)
        val rightBase = Offset(center.x + (dx + halfSpread) * halfW, baseY)
        val arc = Path().apply {
            moveTo(leftBase.x, leftBase.y)
            cubicTo(
                leftBase.x, peak.y + r * 0.1f,
                peak.x - r * 0.12f, peak.y,
                peak.x, peak.y,
            )
            cubicTo(
                peak.x + r * 0.12f, peak.y,
                rightBase.x, peak.y + r * 0.1f,
        ...
        drawPath(
            path = arc,
            brush = Brush.linearGradient(
                colors = listOf(silverMetal.light, silverMetal.mid, silverMetal.dark),
                start = Offset(peak.x, peak.y),
                end = Offset(peak.x, baseY),
            ),
        )
        drawPath(path = arc, color = silverMetal.outline, style = Stroke(width = r * 0.035f))

        drawCircle(color = silverMetal.outline, radius = r * 0.075f, center = peak)
        drawCircle(color = accent, radius = r * 0.055f, center = peak)
    }

    val centralGem = Offset(center.x, center.y - r * 0.78f)
    drawCircle(color = silverMetal.outline, radius = r * 0.16f, center = centralGem)
    drawCircle(
        brush = Brush.radialGradient(
            colors = listOf(Color(0xFFE0B8FF), Color(0xFF9E3BFF)),
            center = Offset(centralGem.x - r * 0.04f, centralGem.y - r * 0.04f),
            radius = r * 0.16f,
        ),
        radius = r * 0.13f,
        center = centralGem,
    ...
    drawFourPointSparkle(
        center = Offset(center.x + halfW * 0.45f, center.y - r * 0.35f),
        size = r * 0.13f,
        color = Color.White,
    )
}
